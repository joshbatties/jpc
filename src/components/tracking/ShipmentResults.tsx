  ...
  return (
    <div className="w-full max-w-2xl lg:max-w-4xl mx-auto font-['Urbanist'] space-y-12">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
        <div>
          <span className="font-medium text-black">Booking Number: </span>
          <span className="text-gray-500">{bookingNumber}</span>
        </div>
        <div>
          <span className="font-medium text-black">
            PO Number{poNumbers.length === 1 ? '' : 's'}: 
          </span>
          <span className="text-gray-500">{poNumbers.join(', ')}</span>
        </div>
        <div>
          <span className="font-medium text-black">
            Container{containers.length === 1 ? '' : 's'}: 
          </span>
          <span className="text-gray-500">{containers.join(', ')}</span>
        </div>
        <div>
          <span className="font-medium text-black">Delivery Address: </span>
          <span className="text-gray-500">{deliveryAddress}</span>
        </div>
      </div>
    </div>
  );
