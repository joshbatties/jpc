...
const CompanyResults: React.FC<CompanyResultsProps> = ({ data = [], customerCode }) => {
  ...
  const formatDateDisplay = (port: string, date: string, isDeparture: boolean): string => {
    if (!date || date === 'undefined' || date === 'null') {
      ...
      return `${isDeparture ? 'Departing' : 'Arriving'} <span class="font-bold text-black">${cityName}</span> on <span class="font-bold text-black">TBA</span>`;
    }
    ...
    if (!day || !month || !year) {
      const cityName = PORT_TRANSLATIONS[port] || port;
      return `${isDeparture ? 'Departing' : 'Arriving'} <span class="font-bold text-black">${cityName}</span> on <span class="font-bold">TBA</span>`;
    }
    ...
    return `<span class="text-gray-900">${action} `
      + `<span class="font-bold text-black">${cityName}</span> on `
      + `<span class="font-bold text-black">${day}/${month}/${year}</span></span>`;
  };
              ...
              <React.Fragment key={item.booking}>
                ...
                >
                  <div className="col-span-2">
                    <div className="font-medium mb-2 text-black">{item.booking}</div>
                ...
                <div 
                  onClick={() => handleRowClick(item.booking)}
                  className="hidden sm:grid grid-cols-4 gap-8 px-6 py-4 hover:bg-gray-50 transition-colors duration-150 cursor-pointer text-black"
                >
                  <div className="font-medium text-black">{item.booking}</div>
                  <div>
                    <span className={`inline-flex items-center px-3 py-1 rounded-xl text-sm text-black
                      ${STATUS_COLOR_CLASSES[item.status].bg}
                      ${STATUS_COLOR_CLASSES[item.status].text}
                      border ${STATUS_COLOR_CLASSES[item.status].border}`}
                    >
